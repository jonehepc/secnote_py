...
<!-- 来自 Plan 02 的 PasswordDialog 接口（含 set_error_message） -->
```python
...
class PasswordDialog(QDialog):
    ...
    def set_error_message(self, text: str)   # D-35: 在对话框内显示错误提示，不关闭对话框
```
...
<task type="auto">
  ...
  <read_first>
    ...
    - 02-CONTEXT.md — D-35（密码错误在对话框内重试）、D-36（保存流程顺序）、D-40~D-44（最近文件管理）
  ...
  <action>
    ...
    **B) 实现 `_on_open_notebook()` — 打开文件完整流程 (FILE-02, D-35, D-36)：**

    替换现有的桩函数。使用循环模式：密码错误时在同一个 PasswordDialog 内显示错误提示，不关闭对话框，允许用户重试 (D-35)。
    ...
    **G) 在 `_setup_central_area()` 中连接欢迎页最近文件信号：**
    ...
    并实现 handler（使用与 _on_open_notebook 相同的密码重试循环模式）：
  ...
  <acceptance_criteria>
    ...
    - `_on_open_notebook` 中使用 `while dialog.exec()` 循环实现密码重试（非递归，非 QMessageBox）
    - `_on_open_notebook` 中调用 `dialog.set_error_message()`
  ...
  <verify>
    <automated>
      ...
      grep -c "set_error_message" src/secnotepad/ui/main_window.py && \
...
- 窗口标题正确反映当前文件路径和脏状态
- 保存/另存为按钮在加载笔记本后启用
- 密码错误时在 PasswordDialog 内显示错误提示，不关闭对话框，允许重试（D-35）
- 所有测试通过
</success_criteria>
