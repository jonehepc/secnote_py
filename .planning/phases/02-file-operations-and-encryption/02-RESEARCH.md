...
1. **CFB 废弃是否影响用户选择 D-24 的 AES-256-CFB?**
   - 已知情况: CRYPT-03 要求 SHA-512 + AES-256-CFB, D-24 锁定为 PBKDF2 + AES-256-CFB + HMAC
   - 不确定: 用户是否知情 CFB 在 cryptography 库中已废弃
   - 建议: 在 plan-check 前通过用户确认。目前使用 `decrepit` 导入路径兼容，但长远（cryptography 49+）可能需要迁移到 AES-256-GCM

2. **密码强度指示器实现方式?**
   - 已知: 使用进度条或文字显示弱/中/强 (D-32)
   - 不确定: 使用 zxcvbn 库还是简易熵值计算
   - 建议: 简易熵值计算（字符集大小 × 长度）对当前场景已足够，避免额外依赖。如用户偏好更准确的评估再引入 zxcvbn

3. **文件服务是独立类还是静态方法?**
   - 已知: 需要加密/解密/保存/打开 4 个核心操作
   - 不确定: 是否需实例化（如维护状态）还是纯静态工具类
   - 建议: 纯静态方法，不维护状态。加密操作是无状态的（每次独立派生密钥）
